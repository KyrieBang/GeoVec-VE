...
void calSpeed(int cpu_rank, int process_num, vector<string> shp_files, linestringNode *RNode, int min_len, int max_len, short min_level, short max_level){
    ...
    while (1){
        ...
        if (tile_node){
            double f_len = tile_node->node_flen;
            if (abs(f_len - 0) <= 1e-9){
                continue;
            }
...
void calSpeed(int cpu_rank, int process_num, vector<string> shp_files, polygonNode *RNode, int min_len, int max_len, short min_level, short max_level){
    map<int, vector<double>> DiPG_time_map, DaPG_time_map;
    for (int i = 0; i < 6; i++){
        DiPG_time_map[i] = vector<double>();
        DaPG_time_map[i] = vector<double>();
...
int main(int argc, char **argv){
    ...
    short min_len = stoi(argv[3]);
    short max_len = stoi(argv[4]);
    ...
    if (getShpFiles(data_path, shp_files)){
        if (shp_type == "linestring"){
            ...
            // calculate the plot speed of DiPG and DaPG on diffirent feature length interval
            calSpeed(cpu_rank, process_num, shp_files, linestringRNode, min_len, max_len, min_level, max_level);
        }
            ...
            // calculate the plot speed of DiPG and DaPG on diffirent feature length interval
            calSpeed(cpu_rank, process_num, shp_files, polygonRNode, min_len, max_len, min_level, max_level);
        }
    }
