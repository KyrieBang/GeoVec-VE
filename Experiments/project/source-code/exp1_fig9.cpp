...
void calSpeed(int cpu_rank, int process_num, vector<string> shp_files, pointNode *RNode, int min_count, int max_count, short min_level, short max_level){
    ...
    while (1){
        ...
        if (tile_node){
            int f_count = tile_node->node_fcount;
            if (f_count == 0){
                continue;
            }
            ...
            else if (f_count > max_count){
                DiPG_time_map[5].push_back(DiPG_time);
                DaPG_time_map[5].push_back(DaPG_time);
            }
            else{
                int ix = static_cast<int>(ceil((f_count-min_count)/100.0));
...
void calSpeed(int cpu_rank, int process_num, vector<string> shp_files, linestringNode *RNode, int min_count, int max_count, short min_level, short max_level){
    map<int, vector<double>> DiPG_time_map, DaPG_time_map;
    for (int i = 0; i < 6; i++){
        DiPG_time_map[i] = vector<double>();
        DaPG_time_map[i] = vector<double>();
    ...
    while (1){
        ...
        if (tile_node){
            int f_count = tile_node->node_scount;
            if (f_count == 0){
                continue;
            }
...
void calSpeed(int cpu_rank, int process_num, vector<string> shp_files, polygonNode *RNode, int min_count, int max_count, short min_level, short max_level){
    map<int, vector<double>> DiPG_time_map, DaPG_time_map;
    for (int i = 0; i < 6; i++){
        DiPG_time_map[i] = vector<double>();
        DaPG_time_map[i] = vector<double>();
    ...
    while (1){
        ...
        if (tile_num >= total_num / process_num){
            for (int i = 1; i < 5; i++){
                if (DiPG_time_map[i].size() == 0){
                    vector<int> tile_id;
                    RNode->searchTile(0, 0, 0, min_count+100*(i-1), max_level, tile_id);
                    if (tile_id.size()){
                        DiPG_time_map[i].push_back(DiPGPlot(tile_id[0], tile_id[1], tile_id[2], 2, RNode));
                        DaPG_time_map[i].push_back(DaPGPlot(tile_id[0], tile_id[1], tile_id[2], 2, RNode, max_level));
                    }
                }
            }
            break;
...
int main(int argc, char **argv){
    ...
    short min_count = stoi(argv[3]);
    short max_count = stoi(argv[4]);
    ...
    if (getShpFiles(data_path, shp_files)){
        if (shp_type == "point"){
            ...
            // calculate the plot speed of DiPG and DaPG on diffirent feature number interval
            calSpeed(cpu_rank, process_num, shp_files, pointRNode, min_count, max_count, min_level, max_level);
        }
            ...
            // calculate the plot speed of DiPG and DaPG on diffirent feature number interval
            calSpeed(cpu_rank, process_num, shp_files, linestringRNode, min_count, max_count, min_level, max_level);
        }
            ...
            // calculate the plot speed of DiPG and DaPG on diffirent feature number interval
            calSpeed(cpu_rank, process_num, shp_files, polygonRNode, min_count, max_count, min_level, max_level);
        }
    }
